...
reference mass, and lever arm. Runtime zero is separate. Manual Tare changes
only runtime zero; Reverse Direction changes only sign; Reset requires explicit
confirmation.
...
Bench Calibration and runtime zero are separate concepts. Saved product
calibration consists of unloaded raw offset, known-load scale, lever arm/crank
length, and torque direction. Runtime code must never overwrite those values.
Automatic Ride Zero can update only the runtime zero, requires a valid Bench
Calibration and a stable 32-sample unloaded window, and locks once a valid ride
begins.
...
unloaded/known-load workflow is implemented in the Calibration page. Repeat
physical validation whenever bridge, wiring, mounting, or crank work could
change its response.
